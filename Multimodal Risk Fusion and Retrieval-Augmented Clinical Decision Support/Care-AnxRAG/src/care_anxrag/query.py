...
_SUBTYPE_PATTERNS: dict[str, tuple[str, ...]] = {
    ...
    "panic_disorder": ("panic attack", "panic disorder", "heart racing", "sudden fear"),
    ...
    "agoraphobia": ("agoraphobia", "afraid to leave", "crowded places"),
    "specific_phobia": ("specific phobia", "phobia", "fear of flying", "fear of spiders"),
    "separation_anxiety": ("separation anxiety", "separation fear"),
...
class QueryAnalyzer:
    def analyze(self, query: str, safety_level: SafetyLevel = SafetyLevel.NORMAL, safety_reason: str | None = None) -> QueryAnalysis:
        ...
        wants_recent = bool(
            re.search(r"\b(latest|recent|new|newest|current|today|202[4-9])\b", normalized)
        )
        ...
        population = None
        if re.search(r"\b(child|children|kid|adolescent|teen|teenager|youth)\b", normalized):
            population = "children_and_adolescents"
        elif re.search(r"\b(pregnant|pregnancy|postpartum|perinatal)\b", normalized):
    ...
    def _intent(query: str) -> QueryIntent:
        if re.search(r"\b(latest|recent|new research|new study|current evidence)\b", query):
            return QueryIntent.RECENT_RESEARCH
        if re.search(r"\b(medicine|medication|drug|ssri|snri|benzodiazepine|dose|side effect)\b", query):
            return QueryIntent.MEDICATION
        if re.search(r"\b(treat|treatment|therapy|cbt|exposure|intervention|recommended)\b", query):
            return QueryIntent.TREATMENT
        if re.search(r"\b(symptom|sign|feel like|heart racing|dizzy|sweating)\b", query):
            return QueryIntent.SYMPTOMS
        if re.search(r"\b(diagnos|do i have|is this anxiety|test for)\b", query):
            return QueryIntent.DIAGNOSIS
        if re.search(r"\b(cause|why|risk factor|trigger)\b", query):
            return QueryIntent.CAUSES
        if re.search(r"\b(coping|cope|self help|self-help|breathing|relaxation|what can i do)\b", query):
